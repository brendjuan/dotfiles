...
kbd.bind({ MODKEY, mod.SHIFT }, "r", function()
    cwc.spawn_with_shell('if pgrep -x wf-recorder > /dev/null; then pkill -INT wf-recorder; else wf-recorder -g "$(slurp)" -f "$HOME/recording_$(date +%Y%m%d_%H%M%S).mp4"; fi')
end, { description = "toggle screen recording (region select)", group = "launcher" })
